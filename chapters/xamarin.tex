...
\paragraph{51-55:} Der Benutzer wird per Popup gefragt, ob der Anruf entgegengenommen werden soll (Abb. \crossref{fig:inc-call}).
\begin{figure}
    \centering
...
\paragraph{111-112:} Wenn der grüne \enquote{Anrufen}-Button gedrückt wird, beginnen \texttt{\_player} und \texttt{\_streamer} damit, die Daten abzurufen bzw. bereitzustellen. Diese Funktionen werden im Abschnitt \crossref{ssec:classes} genauer beschrieben.
...
Ähnlich wie bei \crossref{ssec:app-xaml-cs} wird hier das native Android-Projekt initialisiert.
...
\end{minted}
\paragraph{39-49:} Das in \crossref{ssec:mainpage-xaml-cs} beschriebene Verhalten beim Wechsel der App von und in den Hintergrund benötigt ein vom nativen Projekt ausgelöstes Ereignis.
